...
import { getSender, getSenderFull ,isOnline,getsenderid} from "../config/ChatLogics";
...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  const fetchMessages = async () => {
    ...
    const ison=isOnline(user, selectedChat.users);
   setOnline(ison);
    try {
      const config = {
        headers: {
  ...
  useEffect(() => {
    ...
    // Call fetchStatus with correct parameters when receiving userStatusUpdate event
  ...
  return (
    <>
      {selectedChat ? (
        ...
        <Avatar size="sm" name={getSender(user, selectedChat.users)} />
        <Text ml={2}>{getSender(user, selectedChat.users)}</Text>
        {/* Show online status in a beautiful way
        <Box
          ml={2}
          w="6px"
          h="6px"
